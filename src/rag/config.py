...
class Settings(BaseSettings):
    ...
    @classmethod
    def check_openai_api_key_not_empty(cls, value: str, info) -> str:
        if not value or value.strip() == "":
            logger.error(f"{info.field_name} cannot be empty.")
            raise ValueError(f"{info.field_name} cannot be empty.")
        return value
    
    @field_validator("MONGODB_URI", "MONGODB_DATABASE_NAME")
    @classmethod
    def check_mongodb_fields_not_empty(cls, value: str, info) -> str:
        if not value or value.strip() == "":
            logger.error(f"{info.field_name} cannot be empty.")
            raise ValueError(f"{info.field_name} cannot be empty.")
        return value
